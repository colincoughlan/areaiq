...
5c. **Community services directory** (docs/specs/community-directory.md): babysitters/dog walkers/handymen/other household help, posted by individuals. Contact info is NEVER public — enforced at the DB layer (RLS blocks direct reads of `providers`; a `public_providers` view excludes `contact_email`), contact only via a mediated request. AreaIQ explicitly does not verify identity or run background checks, and says so on every listing. No ratings/reviews, no payments/booking in v1 — deferred, not silently missing. Category allowlist only. Moderation gate before publishing, same as offers.
...
- [~] Phase 2 in progress:
  ...
  - Budget + must-have-amenities matching live at `/find` (`docs/specs/budget-amenity-match.md`, `src/lib/region-match.ts`, `region-match-data.ts`): filters (never scores) the 334 covered places by max median rent (ACS) + must-have amenity categories (added a new `nightlife` category — bars/pubs/nightclubs). Plain GET form, no JS required, no live external calls. Region-wide amenity data ingestion (`npm run ingest:amenities-regions`, resumable, `--limit N` / `--county "X"` for partial/prioritized runs) is IN PROGRESS: 5 of 330 non-pilot regions done so far (acton, alhambra, alondra-park, altadena, arcadia — LA County prioritized per "go deep on LA first"), live-verified "must have" filtering works correctly against this real subset. Rent-only searches already cover all 334 places. Continue with `npm run ingest:amenities-regions -- --county "Los Angeles County"` to keep growing LA coverage, then drop `--county` for the other 4 counties — the public Overpass instance is inconsistently loaded (some regions succeed in ~5s, others 504/406 even on retry), so expect this to take several sessions of incremental runs rather than one sitting.
  - Community services directory live (rule 5c above, `docs/specs/community-directory.md`, `src/lib/community.ts`, `supabase/migrations/0004_community_directory.sql`, `/community`, `CommunityDirectory` on every area/region page): babysitting/pet-care/handyman/other listings, demo mode until Supabase is wired (same pattern as offers/pulse), contact-request relay stored server-side but **actual email delivery to providers isn't built yet** — noted honestly in the API response rather than implied, since that needs a Supabase Edge Function + email provider integration as a follow-up.
  - Remaining, roughly in build order: continue `ingest:amenities-regions` (LA County first, then OC/Riverside/San Bernardino/Ventura); wire actual email delivery for community-directory contact requests once Supabase exists; events/things-to-do data layer (concerts, festivals — scoped to LA first, needs a Ticketmaster or Eventbrite API key); Supabase wiring (auth + saved areas + offers/pulse/community live mode); iOS Capacitor wrapper (needs Xcode/Apple Developer account on a Mac); pre-generated AI narratives; run Redfin market ingestion to completion; app rename (AreaIQ name is taken)
